...
const TripDetailsModal = ({ isOpen, onClose, trip: initialTrip }: TripDetailsModalProps) => {
  ...
  if (isFetchingDetails) {
    return (
      ...
      >
        <div className="bg-white rounded-xl w-full max-w-[400px] flex flex-col items-center justify-center p-10 gap-4">
          <div className="w-10 h-10 border-4 border-[#1DAFA1] border-t-transparent rounded-full animate-spin"></div>
          <span className="text-[#4E616A] font-medium text-[16px]">Fetching trip details...</span>
        </div>
      </div>
    );
  }
                  ...
                  <div className="divide-y divide-[#DFE6E5]">
                    <div className="flex justify-between px-4 py-2.5 text-[14px] font-medium">
                      ...
                      <span className="font-semibold text-[12px] text-[#101828]">
                        £
                        {isCancelled
                          ? (trip.cancellationDetails?.fee || 0).toFixed(2)
                          : (trip.baseFare || 0).toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between px-4 py-2.5 text-[14px] font-medium">
                      <span className="text-[#4E616A]">
                      ...
                      <span className="font-semibold text-[12px] text-[#101828]">
                        £
                        {isCancelled
                          ? (trip.cancellationDetails?.waitingCharge || 0).toFixed(2)
                          : (trip.distanceFare || 0).toFixed(2)}
                      </span>
                    </div>
                    {!isCancelled && (
                      <div className="flex justify-between px-4 py-2.5 text-[14px] font-medium">
