...
const PAGE_TITLES: Record<string, { title: string; subtitle: string }> = {
  ...
  [routes.TRANSACTIONS]: { title: 'Transactions', subtitle: 'Oversee all fleet transactions with comprehensive insights.' },
  [routes.SUPPORT]: { title: 'Support Tickets', subtitle: 'Manage all the Support Tickets' }
};
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            />
            <ChevronDown className={`text-[#2D2D2D] w-4 h-4 transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : ''}`} />
          </div>
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-3 w-[200px] bg-white  shadow-[0_0_16px_0_rgba(237,155,14,0.2)] border border-[#DFE6E5] rounded-lg  py-2 z-50 overflow-hidden">
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-2.5 text-[14px] font-medium text-[#4E616A]  cursor-pointer"
              >
                <img src="/icons/log.svg" alt="profile" className='w-[22px] h-[22px]' />
                Log out
              </button>
            </div>
          )}
