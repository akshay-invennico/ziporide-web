...
import { tripHistoryData, TRIP_ITEMS_PER_PAGE, type TripStatus, type TripRecord } from "../../data/TripHistoryData";
              ...
              {FILTER_TABS.map((tab) => (
                <button
                  ...
                  className={`px-4 py-2  rounded-sm text-[14px] cursor-pointer font-medium transition-all ${activeTab === tab
                    ? tab === "Assigned" || tab === "All" ? " bg-[#EEFFFD] text-[#1DAFA1] border border-[#1DAFA1] "
                      : tab === "In Progress" ? "bg-[#FFF3D4] text-[#F6921E] border border-[#F6921E]"
                        : tab === "Completed" ? " bg-[#EAFFF2] text-[#00A63E] border border-[#00A63E]"
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#F9F9F9] border-b border-[#DFE6E5] text-[14px] font-medium uppercase tracking-wider text-[#4E616A]">
                {["TRIP ID", "RIDER", "DRIVER", "ROUTE", "AMOUNT", "TIME & DATE", "STATUS", "ACTION"].map(
                  (col) => (
                    <th
                      key={col}
