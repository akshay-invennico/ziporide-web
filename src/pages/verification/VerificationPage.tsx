...
const VerificationPage = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "Pending": return "#F6921E"; // Amber
      case "Approved": return "#00A63E"; // Emerald
      case "Rejected": return "#FF0707"; // Red
            ...
            <div className="flex bg-white  gap-2 shrink-0">
              {(["Pending", "Approved", "Rejected"] as const).map(status => (
                <button
                  ...
                  className={`px-4 py-2 rounded-sm text-[14px] cursor-pointer font-medium transition-all ${filterStatus === status
                    ? status === "Pending" ? "bg-[#FFF3D4] text-[#F6921E] border border-[#F6921E]"
                      : status === "Approved" ? "bg-[#EAFFF2] text-[#00A63E] border border-[#00A63E]"
          ...
          <tbody className="text-sm">
            {currentData.length > 0 ? (
              currentData.map((request) => (
                <tr key={request.id} className="border-b border-[#DFE6E5] hover:bg-gray-50/50 transition-colors last:border-b-0">
                  <td className="px-6 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[12px] font-medium text-[#4E616A]">{request.phone}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-3">
                  ...
                  {filterStatus === "Approved" && (
                    <td className="px-6 py-3">
                      <span className="text-[#4E616A] text-[14px] font-medium">{request.actionDate || request.appliedOn}</span>
                    </td>
                  )}
                  <td className="px-6 py-3">
                  ...
                  <td className="px-6 py-3">
                    {request.status === "Pending" ? (
                      ...
                      >
                        <img src="/icons/verification/investigate.svg" alt="investigate" className="w-[24px] h-[24px]" />
                        Investigate
                      </Link>
                    ) : (
                      <Link
                        ...
                        className="flex items-center gap-2 text-[#1DAFA1]  font-medium text-[14px]"
                      >
                        <img src="/icons/verification/eyes.svg" alt="investigate" className="w-[22px] h-[22px]" />
                        View
                      </Link>
                    )}
                  </td>
