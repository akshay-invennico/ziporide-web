...
export const useTransactions = (initialParams: UseTransactionsParams = { page: 1, limit: 12, category: 'All', search: '' }) => {
    ...
    const fetchTransactions = useCallback(async (params: UseTransactionsParams) => {
        ...
        try {
            ...
            const response = await apiClient.get<TransactionResponse>(`${API.TRANSACTIONS}?${queryParams.toString()}`);
            ...
            if (apiResponse && apiResponse.success) {
                const results = apiResponse.data.results.map(item => ({
                    ...
                    driver: item.driver ? {
                        ...item.driver,
                        avatar: item.driver.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(item.driver.name)}&background=1DAFA1&color=fff`
                    } : undefined
                }));
    ...
    }, [fetchTransactions, initialParams.page, initialParams.limit, initialParams.category, initialParams.search]);
